...
*
* DESCRIPTION
* This file contains definitions for miscellaneous retargetable functions
*
* $Revision: 12 $
* $Date: 10/12/21 4:16p $
*
...
void __default_signal_handler(int sig, int type)
{
    svoSetServoOff();
#if MSG_OUT_MODE == MSG_USBPORT
    if ((sig == eSIGFPE) && (type == eFPE_ZERODIVIDE))
    {
        send_msg80(SHOW_DEBUG_MSG,0x700000+eASSERT_DIVISION_BY_ZERO);
    }
    else
    {
        send_msg80(SHOW_DEBUG_MSG,0x700000+eASSERT_EXCEPTION_SIGNAL_RAISE);
    }
#else
    Delay100uS(1);//needed for svoSetServoOff() to complete
#endif
}
